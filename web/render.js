  ...
  load('https://cdn.jsdelivr.net/npm/mapbox-gl@2/dist/mapbox-gl.js')
    ...
    .then(function(){
      ...
      map.on('load',function(){
        ...
        setTimeout(function(){map.resize();map.triggerRepaint();},400);
        setTimeout(function(){map.resize();map.triggerRepaint();post({type:'info',msg:'after-resize tiles='+map.areTilesLoaded()+' size='+map.getCanvas().width+'x'+map.getCanvas().height});},1300);
        // TERRAIN TEMPORARILY OFF (isolation): if the tilted satellite paints
        // now, the 3D terrain mesh was what rendered black in this WebView.
        try{map.setFog({'color':'#dbe4f0','horizon-blend':0.25,'high-color':'#a3c4ea','space-color':'#86b0e0','star-intensity':0});}catch(e){}
