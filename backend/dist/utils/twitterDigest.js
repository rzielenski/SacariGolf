...
exports.composeDigest = composeDigest;
exports.fetchDigestStats = fetchDigestStats;
exports.buildDigestText = buildDigestText;
exports.runDailyDigest = runDailyDigest;
exports.startTwitterDigestSchedule = startTwitterDigestSchedule;
exports.stopTwitterDigestSchedule = stopTwitterDigestSchedule;
/**
 ...
 */
function composeDigest(stats, dateKey) {
    const { roundsToday, ballsLost, ballsFound, topLoser, topFinder, roundOfDay } = stats;
    if (roundsToday === 0 && ballsLost === 0 && ballsFound === 0)
        return null;
    const seed = seedFrom(dateKey);
    const pick = (arr) => arr[seed % arr.length];
    const lines = [`⛳ Sacari recap · ${prettyDate(dateKey)}`];
    // Activity headline.
    if (roundsToday > 0) {
        let s = `${plural(roundsToday, 'round')} logged`;
        if (ballsLost > 0)
            s += `, ${plural(ballsLost, 'ball')} lost to the rough 💦`;
        lines.push(s + '.');
    }
    else if (ballsLost > 0) {
...
async function fetchDigestStats(tz) {
    const totals = await pool_1.default.query(`SELECT
       ...
       (SELECT COUNT(*) FROM ball_log b
         WHERE b.kind = 'found' AND b.created_at >= ${DAY_START})::int AS balls_found`, [tz]);
    const person = async (kind) => {
        const { rows } = await pool_1.default.query(`SELECT u.username, u.share_to_twitter AS opted_in, COUNT(*)::int AS count
         FROM ball_log b
    ...
    const roundRows = await pool_1.default.query(`SELECT u.username, c.course_name AS course, r.total_score AS strokes,
      ...
      WHERE r.created_at >= ${DAY_START}
        ...
        AND array_length(r.hole_scores, 1) >= ${MIN_HOLES}`, [tz]);
    let roundOfDay = null;
    for (const row of roundRows.rows) {
        if (row.played_par == null)
            continue;
        const toPar = row.strokes - row.played_par;
        if (toPar < TO_PAR_FLOOR || toPar > TO_PAR_CEIL)
            continue;
...
async function buildDigestText(tz, dateKey) {
...
function localNow(tz) {
    ...
    if (hour === 24)
        hour = 0; // some ICU builds emit '24' for midnight
...
async function runDailyDigest() {
    ...
    running = true;
    try {
        if (!(0, tweet_1.isConfigured)())
            return; // no creds → stay dormant, don't spam logs
        ...
        if (hour < targetHour)
            return;
        const seen = await pool_1.default.query(`SELECT 1 FROM digest_log WHERE digest_date = $1`, [dateKey]);
        if (seen.rowCount)
            return; // already handled today
        const text = await buildDigestText(tz, dateKey);
        if (text) {
            const tweetId = await (0, tweet_1.postTweet)(text);
            if (!tweetId)
                return; // post failed — leave unrecorded so we retry
            await pool_1.default.query(`INSERT INTO digest_log (digest_date, tweet_id) VALUES ($1, $2)
         ON CONFLICT (digest_date) DO NOTHING`, [dateKey, tweetId]);
            console.log(`[digest] posted ${dateKey} → tweet ${tweetId}`);
        }
        else {
            // Quiet day: record it so we don't recompute every 15 min until midnight.
            await pool_1.default.query(`INSERT INTO digest_log (digest_date, tweet_id) VALUES ($1, NULL)
         ON CONFLICT (digest_date) DO NOTHING`, [dateKey]);
...
function startTwitterDigestSchedule() {
...
function stopTwitterDigestSchedule() {
    if (digestHandle) {
