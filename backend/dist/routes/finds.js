...
router.post('/', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    ...
    try {
        const { rows } = await pool_1.default.query(`INSERT INTO finds (user_id, photo_url, description) VALUES ($1, $2, $3) RETURNING *`, [req.userId, `/uploads/${filename}`, desc || null]);
...
router.get('/pair', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    ...
    const { rows: pool_rows } = await pool_1.default.query(`SELECT f.find_id
     FROM finds f
     ...
     LIMIT 500`, [req.userId]);
    if (pool_rows.length < 2)
    ...
    for (let attempt = 0; attempt < Math.min(8, shuffled.length); attempt++) {
        ...
        const { rows: partnerRows } = await pool_1.default.query(`SELECT find_id FROM finds
        WHERE find_id != $1
        ...
        ORDER BY RANDOM()
        LIMIT 1`, [first, shuffled, req.userId]);
    ...
    await pool_1.default.query(`INSERT INTO find_pair_seen (user_id, find_a_id, find_b_id)
     VALUES ($1, $2, $3)
     ON CONFLICT (user_id, find_a_id, find_b_id) DO NOTHING`, [req.userId, pa, pb]);
    // Hydrate the chosen pair with the full row data the client renders.
    const { rows } = await pool_1.default.query(`SELECT f.find_id, f.photo_url, f.description, f.elo, f.total_votes,
            u.username, u.user_id
     FROM finds f
     JOIN users u ON u.user_id = f.user_id
     WHERE f.find_id = ANY($1::uuid[])`, [[a, b]]);
    // Preserve the picked order so the client can rely on rows[0] / rows[1].
    rows.sort((x, y) => (x.find_id === a ? -1 : 1));
    return res.json(rows);
}));
router.post('/vote', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    ...
    try {
        ...
        const { rows } = await client.query(`SELECT find_id, elo FROM finds WHERE find_id = ANY($1) FOR UPDATE`, [[winnerId, loserId]]);
        if (rows.length < 2) {
        ...
        await client.query(`UPDATE finds SET elo = GREATEST(100, elo + $1), total_votes = total_votes + 1 WHERE find_id = $2`, [delta, winnerId]);
        await client.query(`UPDATE finds SET elo = GREATEST(100, elo - $1), total_votes = total_votes + 1 WHERE find_id = $2`, [delta, loserId]);
...
router.get('/leaderboard', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (_req, res) => {
    const { rows } = await pool_1.default.query(`SELECT f.find_id, f.photo_url, f.description, f.elo, f.total_votes, f.created_at,
            u.username, u.user_id
...
router.get('/mine', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    const { rows } = await pool_1.default.query(`SELECT find_id, photo_url, description, elo, total_votes, created_at
     FROM finds WHERE user_id = $1 ORDER BY elo DESC`, [req.userId]);
    const avgElo = rows.length > 0
        ? Math.round(rows.reduce((s, f) => s + f.elo, 0) / rows.length)
        : null;
    return res.json({ finds: rows, avgElo });
}));
router.post('/:id/report', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    const reason = String(req.body?.reason ?? '').trim().slice(0, 500);
    if (!reason)
        return res.status(400).json({ error: 'reason required' });
    await pool_1.default.query(`INSERT INTO find_reports (find_id, reporter_id, reason)
     VALUES ($1, $2, $3) ON CONFLICT (find_id, reporter_id) DO NOTHING`, [req.params.id, req.userId, reason]);
    return res.json({ success: true });
}));
router.delete('/:id', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    const { rows } = await pool_1.default.query(`DELETE FROM finds WHERE find_id = $1 AND user_id = $2 RETURNING photo_url`, [req.params.id, req.userId]);
    if (!rows.length)
        return res.status(404).json({ error: 'Find not found or not yours' });
