...
if (!fs_1.default.existsSync(FEED_DIR))
    fs_1.default.mkdirSync(FEED_DIR, { recursive: true });
...
function unlinkFeedImage(url) {
    if (!url?.startsWith('/uploads/feed/'))
        return;
    const fname = url.replace('/uploads/feed/', '');
    try {
        fs_1.default.unlinkSync(path_1.default.join(FEED_DIR, fname));
    }
...
router.post('/', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    ...
    if (typeof imageBase64 === 'string' && imageBase64.length > 0) {
        ...
        fs_1.default.writeFileSync(path_1.default.join(FEED_DIR, filename), buffer);
...
router.post('/:id/report', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    ...
    const { rows } = await pool_1.default.query(`SELECT user_id FROM posts WHERE post_id = $1`, [req.params.id]);
    if (!rows.length)
        return res.status(404).json({ error: 'Post not found' });
    if (rows[0].user_id === req.userId) {
        return res.status(400).json({ error: 'You cannot report your own post' });
    }
    await pool_1.default.query(`INSERT INTO post_reports (post_id, reporter_id, reason)
     VALUES ($1, $2, $3)
     ON CONFLICT (post_id, reporter_id) DO NOTHING`, [req.params.id, req.userId, reason]);
...
router.delete('/:id', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    const { rows } = await pool_1.default.query(`SELECT image_url FROM posts WHERE post_id = $1 AND user_id = $2`, [req.params.id, req.userId]);
    if (!rows.length)
        return res.status(404).json({ error: 'Post not found' });
    ...
    await pool_1.default.query(`DELETE FROM posts WHERE post_id = $1`, [req.params.id]);
 ...
 */
router.get('/feed', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    ...
    const scope = scopeRaw === 'friends' || scopeRaw === 'local' ? scopeRaw : 'global';
    ...
    if (scope === 'local') {
        ...
        if (centerLat == null) {
            const { rows: hc } = await pool_1.default.query(`SELECT c.latitude, c.longitude
           FROM users u
    ...
    const { rows } = await pool_1.default.query(`WITH my_friends AS (
...
router.post('/:id/comments', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    ...
    const { rows: postRows } = await pool_1.default.query(`SELECT p.user_id AS owner_id, owner.push_token, owner.username AS owner_name,
      ...
      WHERE p.post_id = $1`, [req.params.id, req.userId]);
    if (!postRows.length)
        return res.status(404).json({ error: 'post not found' });
    ...
    if (post.owner_id !== req.userId && post.push_token) {
        ...
        (0, notify_1.sendPush)([post.push_token], `${post.actor_name} commented on your post`, preview, { type: 'post_comment', postId: req.params.id, commentId: rows[0].comment_id, fromUserId: req.userId }).catch(() => { });
    }
...
router.delete('/:id/comments/:commentId', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    const { rowCount } = await pool_1.default.query(`DELETE FROM post_comments c
      USING posts p
      WHERE c.comment_id = $1
        AND c.post_id = $2
        AND p.post_id = c.post_id
        AND (c.user_id = $3 OR p.user_id = $3)`, [req.params.commentId, req.params.id, req.userId]);
    if (!rowCount)
