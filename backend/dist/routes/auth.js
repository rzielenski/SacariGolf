...
router.post('/register', async (req, res) => {
    ...
    try {
        const hash = await bcryptjs_1.default.hash(p, 12);
        // Generate a verification code at registration time and store its hash.
        const verifyCode = String(crypto_1.default.randomInt(0, 1000000)).padStart(6, '0');
        const verifyHash = crypto_1.default.createHash('sha256').update(verifyCode).digest('hex');
        ...
        let inviterId = null;
        if (rc) {
            const { rows: invRows } = await pool_1.default.query(`SELECT user_id FROM users WHERE referral_code = $1 LIMIT 1`, [rc]);
        ...
        const { rows } = await pool_1.default.query(`INSERT INTO users (username, email, password_hash,
       ...
       VALUES ($1, $2, $3, $4, NOW() + INTERVAL '24 hours',
               upper(substring(translate(encode(gen_random_bytes(8), 'base64'),
                               ...
                               1, 7)),
               $5)
       RETURNING user_id, username, email, elo, total_matches, total_wins, created_at, email_verified, referral_code`, [u, e, hash, verifyHash, inviterId]);
        ...
        const emailResult = await Promise.race([
            ...
            new Promise((resolve) => setTimeout(() => resolve({ ok: false, error: 'email_send_timeout' }), 8000)),
        ]);
        if (!emailResult.ok) {
        ...
        if (inviterId) {
            try {
                await pool_1.default.query(`INSERT INTO user_perks (user_id, perk_type, earned_reason)
           VALUES ($1, 'lucky_round', 'referral')`, [inviterId]);
                const { rows: tokRows } = await pool_1.default.query(`SELECT push_token FROM users WHERE user_id = $1`, [inviterId]);
                const token = tokRows[0]?.push_token;
                if (token) {
                    await (0, notify_1.sendPush)([token], 'New referral!', `${u} signed up with your code. Lucky Round added to your perks.`, { type: 'referral', referredUserId: rows[0].user_id });
                }
            }
            catch (referralErr) {
                // eslint-disable-next-line no-console
...
router.post('/login', async (req, res) => {
    ...
    try {
        const { rows } = await pool_1.default.query(`SELECT user_id, username, email, password_hash, elo, total_matches, total_wins, avatar_url, created_at,
              is_premium, premium_since, premium_until, premium_plan
       FROM users WHERE email = $1`, [e]);
        ...
        const ok = await bcryptjs_1.default.compare(p, hashToCheck);
...
router.post('/forgot', async (req, res) => {
    ...
    try {
        const { rows } = await pool_1.default.query(`SELECT user_id, password_hash FROM users WHERE email = $1`, [email]);
        const user = rows[0];
        ...
        if (user && user.password_hash) {
            ...
            await pool_1.default.query(`UPDATE users SET reset_code_hash = $1, reset_code_expires_at = NOW() + INTERVAL '15 minutes'
         WHERE user_id = $2`, [codeHash, user.user_id]);
...
router.post('/reset', async (req, res) => {
    ...
    try {
        ...
        const newHash = await bcryptjs_1.default.hash(password, 12);
       ...
       RETURNING user_id, username, email, elo, total_matches, total_wins, avatar_url, created_at`, [newHash, email, codeHash]);
        if (!rows.length)
...
router.post('/verify-email', auth_1.requireAuth, async (req, res) => {
    const ip = clientIp(req);
    ...
    try {
       ...
       RETURNING user_id`, [req.userId, codeHash]);
        if (!rows.length) {
            // Maybe already verified — surface a useful response either way.
            const { rows: check } = await pool_1.default.query(`SELECT email_verified FROM users WHERE user_id = $1`, [req.userId]);
...
router.post('/resend-verification', auth_1.requireAuth, async (req, res) => {
    const ip = clientIp(req);
    ...
    try {
        const { rows } = await pool_1.default.query(`SELECT email, email_verified FROM users WHERE user_id = $1`, [req.userId]);
        if (!rows.length)
       ...
       WHERE user_id = $2`, [codeHash, req.userId]);
        await (0, email_1.sendEmail)({
            to: rows[0].email,
            subject: 'Verify your Sacari account',
