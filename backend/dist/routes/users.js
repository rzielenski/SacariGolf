...
exports.getBlockedIds = getBlockedIds;
...
router.get('/me', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
...
router.patch('/me', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    ...
    if (pushToken !== undefined) {
        ...
        updates.push(`push_token = $${values.length}`);
    }
    if (handicapIndex !== undefined) {
        ...
        if (isNaN(hi) || hi < -10 || hi > 54)
            return res.status(400).json({ error: 'handicapIndex must be between -10 and 54' });
        values.push(hi);
        updates.push(`handicap_index = $${values.length}`);
    }
    if (bio !== undefined) {
    ...
    if (homeCourseId !== undefined) {
        values.push(homeCourseId || null);
        updates.push(`home_course_id = $${values.length}`);
    }
    if (username !== undefined) {
        if (!/^[a-zA-Z0-9_]{3,20}$/.test(username)) {
            return res.status(400).json({ error: 'Username must be 3–20 characters: letters, numbers, or underscores' });
        }
        const { rows: existing } = await pool_1.default.query(`SELECT 1 FROM users WHERE username = $1 AND user_id != $2`, [username, req.userId]);
        if (existing.length)
    ...
    if (theme !== undefined) {
        if (theme === null) {
            ...
            values.push(trackId.slice(0, 64));
            updates.push(`theme_track_id = $${values.length}`);
            values.push(title.slice(0, 200));
            updates.push(`theme_track_title = $${values.length}`);
            values.push(artist.slice(0, 200));
            updates.push(`theme_track_artist = $${values.length}`);
            values.push((artworkUrl ?? '').slice(0, 500) || null);
            updates.push(`theme_track_artwork = $${values.length}`);
            values.push(previewUrl.slice(0, 500));
            updates.push(`theme_track_preview = $${values.length}`);
        }
        else {
    ...
    await pool_1.default.query(`UPDATE users SET ${updates.join(', ')} WHERE user_id = $${values.length}`, values);
...
router.get('/search', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    ...
    const { rows } = await pool_1.default.query(`SELECT user_id, username, elo, avatar_url FROM users
...
router.get('/me/friends', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    const { rows } = await pool_1.default.query(`SELECT DISTINCT ON (u.user_id) u.user_id, u.username, u.elo, u.avatar_url, f.status,
     ...
     ORDER BY u.user_id`, [req.userId]);
    return res.json(rows);
}));
router.get('/me/friend-requests', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    const { rows } = await pool_1.default.query(`SELECT u.user_id, u.username, u.elo, u.avatar_url, f.created_at
     FROM friends f JOIN users u ON u.user_id = f.user_id
     WHERE f.friend_id = $1 AND f.status = 'pending'`, [req.userId]);
    return res.json(rows);
}));
router.post('/me/friends/request', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    ...
    const { rows: targetRows } = await pool_1.default.query(`SELECT 1 FROM users WHERE user_id = $1`, [friendId]);
    ...
    const { rows: existing } = await pool_1.default.query(`SELECT user_id, friend_id, status FROM friends
      WHERE (user_id = $1 AND friend_id = $2)
         OR (user_id = $2 AND friend_id = $1)`, [req.userId, friendId]);
    if (existing.length > 0) {
        ...
        const incoming = existing.find((r) => r.status === 'pending' && r.user_id === friendId && r.friend_id === req.userId);
        if (incoming) {
        ...
        const outgoing = existing.find((r) => r.status === 'pending' && r.user_id === req.userId && r.friend_id === friendId);
        if (outgoing) {
    ...
    await pool_1.default.query(`INSERT INTO friends (user_id, friend_id, status) VALUES ($1, $2, 'pending')
     ON CONFLICT DO NOTHING`, [req.userId, friendId]);
    const { rows } = await pool_1.default.query(`SELECT u.push_token, u2.username AS from_name
     FROM users u, users u2
     WHERE u.user_id = $1 AND u2.user_id = $2`, [friendId, req.userId]);
    if (rows[0]?.push_token) {
        await (0, notify_1.sendPush)([rows[0].push_token], 'Friend Request', `${rows[0].from_name} sent you a friend request!`, { type: 'friendRequest' });
    }
    return res.json({ success: true });
}));
router.post('/me/friends/accept', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    const { friendId } = req.body ?? {};
    if (!friendId)
        return res.status(400).json({ error: 'friendId required' });
    ...
    const { rows } = await pool_1.default.query(`UPDATE friends SET status = 'accepted'
     WHERE user_id = $1 AND friend_id = $2 AND status = 'pending'
     RETURNING user_id`, [friendId, req.userId]);
    if (!rows.length)
...
router.get('/:id/stats', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    const { rows } = await pool_1.default.query(`SELECT r.round_id, r.created_at, r.hole_scores, r.hole_stats, r.total_score,
    ...
    const { rows: userRows } = await pool_1.default.query(`SELECT handicap_index FROM users WHERE user_id = $1`, [req.params.id]);
    const handicapIndex = typeof userRows[0]?.handicap_index === 'number'
    ...
    let sgTotal = 0;
    for (const r of rows) {
        if (!Array.isArray(r.hole_scores) || r.hole_scores.length === 0)
            continue;
        roundsCount += 1;
        const stats = Array.isArray(r.hole_stats) ? r.hole_stats : [];
    ...
    const round = (n, places = 2) => Math.round(n * Math.pow(10, places)) / Math.pow(10, places);
    return res.json({
 ...
 */
router.get('/:id/hole-shots', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    ...
    const params = [req.params.id, courseId, holeNum];
    ...
    const { rows } = await pool_1.default.query(`SELECT s.match_id, m.created_at,
            json_agg(
              json_build_object(
                'club', s.club,
                'lie',  s.lie,
 ...
 */
router.get('/:id/club-stats', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    ...
    const eachShotSegment = (rawShots) => {
        if (!rawShots.length)
            return [];
        if (rawShots[0]?.start && rawShots[0]?.end) {
            return rawShots
                .filter((s) => s?.start && s?.end && typeof s.club === 'string')
                .map((s) => ({
        ...
        for (let i = 0; i < rawShots.length - 1; i++) {
            ...
            if (typeof cur?.lat !== 'number' || typeof nxt?.lat !== 'number')
                continue;
            if (typeof cur.club !== 'string')
                continue;
            out.push({
    ...
    for (const row of rows) {
        ...
        for (const seg of segments) {
            ...
            if (dist_m < 1 || dist_m > 500)
                continue; // sanity: drop GPS noise / impossibly long
 ...
 */
router.get('/:id/shot-stats', auth_1.requireAuth, auth_1.requirePremium, (0, asyncHandler_1.wrap)(async (req, res) => {
    ...
    const { rows } = await pool_1.default.query(`SELECT s.match_id, s.hole_num, s.shot_index,
    ...
    const toRad = (d) => d * Math.PI / 180;
    const distYds = (lat1, lng1, lat2, lng2) => {
    ...
    const { rows: roundRows } = await pool_1.default.query(`SELECT hole_stats FROM rounds
      WHERE user_id = $1
    ...
    for (const r of roundRows) {
        ...
        for (const hs of holeStats) {
            const dists = Array.isArray(hs?.puttDistances)
                ? hs.puttDistances.filter((d) => typeof d === 'number' && d > 0)
                : [];
            if (!dists.length)
                continue;
 ...
 */
router.delete('/me/shots/:shotId', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    const { shotId } = req.params;
    if (!shotId)
        return res.status(400).json({ error: 'shotId required' });
    const { rowCount } = await pool_1.default.query(`DELETE FROM shots WHERE shot_id = $1 AND user_id = $2`, [shotId, req.userId]);
    if (!rowCount)
 ...
 */
router.get('/:id/sg-advanced', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    ...
    const { rows } = await pool_1.default.query(`SELECT s.match_id, s.hole_num,
            json_agg(
              json_build_object(
                'club',  s.club,
                'lie',   s.lie,
    ...
    const toSegments = (raw) => {
        if (!raw.length)
            return [];
        if (raw[0]?.start && raw[0]?.end) {
            return raw
                .filter((s) => s?.start && s?.end)
                .map((s) => ({ start: s.start, end: s.end, club: s.club, lie: s.lie }));
        }
        const out = [];
    ...
    for (const row of rows) {
        ...
        const holeMeta = holes.find((h) => h.hole_num === row.hole_num);
        if (!holeMeta || holeMeta.pin_lat == null || holeMeta.pin_lng == null)
            continue;
        ...
        for (let i = 0; i < segments.length; i++) {
            ...
            const startLie = seg.lie ?? (i === 0 ? 'tee' : 'fairway');
    ...
    const totals = (0, sg_1.aggregateSG)(allShots);
...
router.get('/:id/course-records', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    const { rows } = await pool_1.default.query(`WITH ranked AS (
     ...
     ORDER BY total_score ASC`, [req.params.id]);
    return res.json(rows);
}));
...
router.get('/:id/insights', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    ...
    const { rows: holeRows } = await pool_1.default.query(`WITH src AS (
     ...
     FROM src s,
          ...
          LEFT JOIN holes h ON h.teebox_id = s.teebox_id AND h.hole_num = scored.hole_num`, [userId]);
    ...
    const holeAgg = {};
    const courseRoundCount = {};
    const seenRounds = new Set();
    let eagles = 0, birdies = 0, pars = 0, bogeys = 0, doubles = 0;
    for (const row of holeRows) {
        if (!row.hole_par || !row.score)
            continue;
        const par = row.hole_par;
        if (parBuckets[par]) {
    ...
    const avgPerPar = Object.entries(parBuckets).reduce((acc, [par, b]) => {
    ...
    const { rows: trendRows } = await pool_1.default.query(`SELECT r.round_id,
            r.total_score
              - ROUND(t.par::numeric
...
async function getBlockedIds(userId) {
    if (!userId)
        return [];
    const { rows } = await pool_1.default.query(`SELECT blocked_id FROM blocked_users WHERE blocker_id = $1`, [userId]);
    return rows.map((r) => r.blocked_id);
}
// List my blocks — used by a "Blocked users" settings screen.
router.get('/me/blocks', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    const { rows } = await pool_1.default.query(`SELECT b.blocked_id, b.created_at, b.reason,
            u.username, u.elo, u.avatar_url
     FROM blocked_users b
     JOIN users u ON u.user_id = b.blocked_id
     WHERE b.blocker_id = $1
     ORDER BY b.created_at DESC`, [req.userId]);
    return res.json(rows);
}));
...
router.post('/me/blocks/:userId', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    ...
    await pool_1.default.query(`INSERT INTO blocked_users (blocker_id, blocked_id, reason)
     VALUES ($1, $2, $3)
     ON CONFLICT (blocker_id, blocked_id) DO UPDATE SET reason = EXCLUDED.reason`, [req.userId, req.params.userId, reason]);
    // Tear down any existing friendship (either direction) so the blocked user
    // can't continue to see the blocker as a friend.
    await pool_1.default.query(`DELETE FROM friends
     WHERE (user_id = $1 AND friend_id = $2)
        OR (user_id = $2 AND friend_id = $1)`, [req.userId, req.params.userId]);
    return res.json({ success: true });
}));
// Unblock — also idempotent.
router.delete('/me/blocks/:userId', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    await pool_1.default.query(`DELETE FROM blocked_users WHERE blocker_id = $1 AND blocked_id = $2`, [req.userId, req.params.userId]);
    return res.json({ success: true });
}));
router.get('/me/perks', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    const { rows } = await pool_1.default.query(`SELECT perk_id, perk_type, earned_at, earned_match_id
     FROM user_perks
     WHERE user_id = $1 AND consumed_at IS NULL
     ORDER BY earned_at ASC`, [req.userId]);
    return res.json(rows);
}));
/**
 ...
 */
router.get('/me/referral', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    const { rows: codeRows } = await pool_1.default.query(`SELECT referral_code FROM users WHERE user_id = $1`, [req.userId]);
    const code = codeRows[0]?.referral_code;
    if (!code) {
        // Backfill missed this row somehow — generate one now so the screen
        // still works. Idempotent: UPDATE only if still null.
        const { rows: gen } = await pool_1.default.query(`UPDATE users
       ...
       RETURNING referral_code`, [req.userId]);
        if (gen[0]?.referral_code)
            (codeRows[0] ?? (codeRows[0] = {})).referral_code = gen[0].referral_code;
    }
    const finalCode = codeRows[0]?.referral_code;
    const { rows: stats } = await pool_1.default.query(`SELECT
       (SELECT COUNT(*)::int FROM users
         WHERE referred_by_user_id = $1) AS referred_count,
       (SELECT COUNT(*)::int FROM user_perks
         WHERE user_id = $1 AND earned_reason = 'referral') AS perks_earned`, [req.userId]);
...
router.get('/leaderboard', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    ...
    const rawMode = req.query.mode || 'all';
...
router.get('/:id', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    ...
    const { rows: recentRounds } = await pool_1.default.query(`SELECT r.round_id, r.match_id, r.total_score, r.created_at, r.hole_scores, r.hole_stats,
    ...
    const { rows: followCounts } = await pool_1.default.query(`SELECT
       ...
       (SELECT COUNT(*)::int FROM (
        ...
        ) t) AS followers_count`, [req.params.id]);
    ...
    if (req.params.id === req.userId) {
        ...
        const { rows: fs } = await pool_1.default.query(`SELECT user_id, friend_id, status FROM friends
        WHERE (user_id = $1 AND friend_id = $2)
           OR (user_id = $2 AND friend_id = $1)`, [req.userId, req.params.id]);
        if (fs.length > 0) {
            const accepted = fs.find((r) => r.status === 'accepted');
            if (accepted)
                friendshipStatus = 'friends';
            else if (fs.find((r) => r.user_id === req.userId))
...
router.get('/:id/following', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    const { rows } = await pool_1.default.query(`SELECT DISTINCT ON (u.user_id)
      ...
      ORDER BY u.user_id, x.created_at DESC`, [req.params.id]);
...
router.delete('/me', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    ...
    try {
        ...
        const { rows: ledClans } = await client.query(`SELECT clan_id FROM clan_members WHERE user_id = $1 AND role = 'leader' FOR UPDATE`, [req.userId]);
        for (const c of ledClans) {
         ...
         LIMIT 1`, [c.clan_id, req.userId]);
            if (heir.length) {
                await client.query(`UPDATE clan_members SET role = 'leader' WHERE clan_id = $1 AND user_id = $2`, [c.clan_id, heir[0].user_id]);
            }
            else {
...
router.get('/:id/active-round', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    const { rows } = await pool_1.default.query(`SELECT mp.match_id, mp.user_id, mp.teebox_id, mp.side,
    ...
    if (req.userId !== req.params.id) {
        const { rows: shareRows } = await pool_1.default.query(`SELECT side FROM match_players WHERE match_id = $1 AND user_id = $2`, [active.match_id, req.userId]);
        if (shareRows.length && shareRows[0].side !== active.side) {
            return res.json(null);
        }
    }
    // Normalise empty arrays so the frontend can safely call .length on them.
    if (!active.hole_scores)
        active.hole_scores = [];
    if (!active.hole_stats)
        active.hole_stats = [];
    if (!active.hole_pars)
        active.hole_pars = [];
    ...
    if (!active.round_id && active.match_id && active.user_id && active.teebox_course_id) {
        try {
            const { rows: created } = await pool_1.default.query(`INSERT INTO rounds (match_id, user_id, course_id, teebox_id, hole_scores, round_type)
         VALUES ($1, $2, $3, $4, '{}', 'live')
         ON CONFLICT (match_id, user_id) DO UPDATE SET match_id = EXCLUDED.match_id
         RETURNING round_id, created_at`, [active.match_id, active.user_id, active.teebox_course_id, active.teebox_id]);
            if (created.length) {
                active.round_id = created[0].round_id;
                // If we just created it, populate round_started_at from the new row.
                if (!active.round_started_at)
                    active.round_started_at = created[0].created_at;
            }
        }
 ...
 */
router.post('/me/import-shots', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    ...
    for (const s of shots) {
        ...
        if (!club || !ALLOWED_CLUBS.has(club))
            continue;
        const dist = Number(s?.distance_yds);
        if (!Number.isFinite(dist) || dist < 5 || dist > 500)
            continue;
...
// Avatar upload
router.post('/me/avatar', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    ...
    await pool_1.default.query(`UPDATE users SET avatar_url = $1 WHERE user_id = $2`, [avatarUrl, req.userId]);
...
router.get('/me/notifications', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    const notes = [];
    // Get user's seen-at timestamp for unread calculation
    const { rows: seenRows } = await pool_1.default.query(`SELECT notifications_seen_at FROM users WHERE user_id = $1`, [req.userId]);
    const seenAt = seenRows[0]?.notifications_seen_at ?? new Date(0);
    // Pending friend requests (3-day window)
    const { rows: frs } = await pool_1.default.query(`SELECT u.user_id, u.username, f.created_at FROM friends f
     JOIN users u ON u.user_id = f.user_id
     WHERE f.friend_id = $1 AND f.status = 'pending'
       AND f.created_at > NOW() - INTERVAL '3 days'
     ORDER BY f.created_at DESC LIMIT 10`, [req.userId]);
    for (const r of frs)
        notes.push({ type: 'friend_request', title: 'Friend Request', body: `${r.username} sent you a friend request`, data: { userId: r.user_id }, created_at: r.created_at });
    // Pending match invites (3-day window)
    const { rows: mis } = await pool_1.default.query(`SELECT mi.invite_id, mi.match_id, mi.created_at, u.username AS from_name, m.match_type
     ...
     ORDER BY mi.created_at DESC LIMIT 10`, [req.userId]);
    for (const r of mis)
        notes.push({ type: 'match_invite', title: 'Match Invite', body: `${r.from_name} invited you to a ${r.match_type} match`, data: { matchId: r.match_id, inviteId: r.invite_id }, created_at: r.created_at });
    // Pending clan invites (3-day window)
    try {
        const { rows: cis } = await pool_1.default.query(`SELECT ci.invite_id, ci.clan_id, ci.created_at, u.username AS from_name, c.name AS clan_name
       ...
       ORDER BY ci.created_at DESC LIMIT 10`, [req.userId]);
        for (const r of cis)
            notes.push({ type: 'clan_invite', title: 'Team Invite', body: `${r.from_name} invited you to join ${r.clan_name}`, data: { clanId: r.clan_id, inviteId: r.invite_id }, created_at: r.created_at });
    }
    catch { /* table may not exist yet */ }
    // Recent match results (3-day window)
    const { rows: mrs } = await pool_1.default.query(`SELECT mr.match_id, mr.winner_side, mr.delta_elo, mr.created_at, m.match_type, mp.side AS my_side
     ...
     ORDER BY mr.created_at DESC LIMIT 10`, [req.userId]);
    for (const r of mrs) {
    ...
    try {
        const { rows: mentions } = await pool_1.default.query(`SELECT pm.post_id, pm.created_at, au.username AS from_name
        ...
        ORDER BY pm.created_at DESC LIMIT 10`, [req.userId]);
        for (const r of mentions)
            notes.push({ type: 'mention', title: 'You were tagged', body: `${r.from_name} tagged you in a post`, data: { postId: r.post_id }, created_at: r.created_at });
    }
    ...
    try {
        const { rows: chatRows } = await pool_1.default.query(`WITH dm_unread AS (
         SELECT 1 AS n
...
router.post('/me/notifications/seen', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    await pool_1.default.query(`UPDATE users SET notifications_seen_at = NOW() WHERE user_id = $1`, [req.userId]);
    return res.json({ success: true });
}));
/**
 ...
 */
router.post('/me/drinks', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    ...
    const { rows } = await pool_1.default.query(`UPDATE users
        SET drinks = LEAST(100000, GREATEST(0, drinks + $2))
      WHERE user_id = $1
      RETURNING drinks`, [req.userId, delta]);
