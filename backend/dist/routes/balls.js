...
async function totalsFor(userId) {
    const { rows } = await pool_1.default.query(`SELECT COUNT(*) FILTER (WHERE kind = 'found')::int AS found,
            COUNT(*) FILTER (WHERE kind = 'lost')::int  AS lost
       FROM ball_log WHERE user_id = $1`, [userId]);
...
router.get('/me', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    ...
    const { rows: recent } = await pool_1.default.query(`SELECT log_id, kind, created_at
       FROM ball_log WHERE user_id = $1
      ORDER BY created_at DESC LIMIT 30`, [req.userId]);
    return res.json({ ...totals, recent });
}));
router.post('/log', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    ...
    if (clientId) {
        await pool_1.default.query(`INSERT INTO ball_log (user_id, kind, client_id)
       VALUES ($1, $2, $3)
       ON CONFLICT (user_id, client_id) WHERE client_id IS NOT NULL DO NOTHING`, [req.userId, kind, clientId]);
    }
    else {
        await pool_1.default.query(`INSERT INTO ball_log (user_id, kind) VALUES ($1, $2)`, [req.userId, kind]);
    }
    return res.json(await totalsFor(req.userId));
}));
router.post('/undo', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    const clientId = typeof req.body?.clientId === 'string' && req.body.clientId.length > 0
        ? req.body.clientId.slice(0, 64) : null;
    ...
    if (clientId) {
        const { rows: prev } = await pool_1.default.query(`SELECT 1 FROM ball_log_undo WHERE user_id = $1 AND client_id = $2`, [req.userId, clientId]);
        if (prev.length > 0) {
            return res.json(await totalsFor(req.userId));
        }
    }
    ...
    try {
        ...
        const { rows: deleted } = await client.query(`DELETE FROM ball_log
        WHERE log_id = (
       ...
       RETURNING log_id`, [req.userId]);
        if (clientId) {
            await client.query(`INSERT INTO ball_log_undo (user_id, client_id, deleted_log_id)
         VALUES ($1, $2, $3)
         ON CONFLICT (user_id, client_id) DO NOTHING`, [req.userId, clientId, deleted[0]?.log_id ?? null]);
        }
...
router.get('/leaderboard', auth_1.requireAuth, (0, asyncHandler_1.wrap)(async (req, res) => {
    ...
    const { rows } = await pool_1.default.query(`${friendsOnly ? friendScopeCte : ''}
