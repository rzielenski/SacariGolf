...
/** A round + the rating/slope context needed for a USGA score differential. */
...
/** The fields needed to score one round. par/numHoles drive the no-rating
 *  fallback; the rating/slope context (optional) drives the difficulty-adjusted
 *  differential when present. Mirrors the columns roundScore.ts reads. */
export interface RoundForScore {
  totalScore: number;
  /** Teebox par, for its num_holes holes (used by the fallback). */
  par: number;
  /** Teebox hole count (9 or 18). */
  numHoles: number;
  /** Holes actually played this round; defaults to numHoles when unknown. */
  holesPlayed?: number | null;
  // --- Course-difficulty context (optional). When courseRating + slopeRating
  //     are both present, the score is a rating/slope-adjusted differential. ---
  courseRating?: number | null;
  slopeRating?: number | null;
  /** 'front' | 'back' | 'full' — which nine a 9-on-18 round covered. */
  holesSubset?: string | null;
 ...
 */
export function normalizedScore(round: RoundForScore): number {
  ...
  if (!round.numHoles || !hp) return round.totalScore - round.par;
 ...
 */
export function rankByScore<T extends RoundForScore>(entries: T[]): T[] {
  return entries
