...
function kFactor(totalMatches: number, elo: number): number {
...
function seasonIdForDate(d: Date): string {
  const y = d.getUTCFullYear();
  const m = d.getUTCMonth(); // 0 = Jan
  if (m >= 4 && m <= 9) return `${y}-summer`;
  const sy = m >= 10 ? y : y - 1;
  return `${sy}-winter`;
}
...
export async function replayAllElo(): Promise<ReplaySummary> {
  ...
  try {
    ...
    if (backupTaken) {
      await client.query(
        `INSERT INTO elo_backup_users (user_id, elo, total_matches, total_wins, total_ties)
         SELECT user_id, elo, total_matches, total_wins, total_ties FROM users`);
      await client.query(
        `INSERT INTO elo_backup_results (match_id, delta_elo, details)
         SELECT match_id, delta_elo, details FROM match_results`);
    }

    // ── 2. Load every resolved, non-practice match in order ─────────
    const { rows: matches } = await client.query(
      `SELECT mr.match_id, mr.match_type, mr.winner_side,
    ...
    const tm = new Map<string, number>();   // total matches
    const tw = new Map<string, number>();   // total wins
    const tt = new Map<string, number>();   // total ties
    const place = new Map<string, number>(); // placement count THIS season
    const E = (u: string) => elo.get(u) ?? FLOOR;
    const TM = (u: string) => tm.get(u) ?? 0;
    const isPlacement = (u: string) => (place.get(u) ?? 0) < PLACEMENT_MATCHES;
    const bump = (u: string, d: number, won: boolean, tie: boolean) => {
      elo.set(u, Math.max(FLOOR, E(u) + d));
      tm.set(u, TM(u) + 1);
      if (won) tw.set(u, (tw.get(u) ?? 0) + 1);
      if (tie) tt.set(u, (tt.get(u) ?? 0) + 1);
      place.set(u, (place.get(u) ?? 0) + 1);
    };

    // Per-match writeback (new delta_elo + details patch).
    const writeback = new Map<string, { delta_elo: number; patch: any }>();
    const processed = new Set<string>();
    let curSeason: string | null = null;
    ...
    const sidesFor = async (matchId: string, details: any): Promise<{ s1: string[]; s2: string[] }> => {
      ...
      const { rows } = await client.query(
        `SELECT user_id, side FROM match_players WHERE match_id = $1 ORDER BY side, joined_at`,
        [matchId]);
      return {
        s1: rows.filter((r: any) => r.side === 1).map((r: any) => r.user_id),
        s2: rows.filter((r: any) => r.side !== 1).map((r: any) => r.user_id),
      };
    };

    for (const mt of matches) {
      if (processed.has(mt.match_id)) continue;
      ...
      try {
        if (isFFA) {
          ...
          const placements: any[] = Array.isArray(details.placements) ? details.placements : [];
          if (placements.length < 2) { matchesSkipped++; continue; }
          const ids: string[] = placements.map((p) => p.user_id);
          const diff = new Map<string, number>(placements.map((p) => [p.user_id, p.diff ?? 0]));
          const N = ids.length;
          const raw = new Map<string, number>(ids.map((u) => [u, 0]));
          const wins = new Map<string, number>(ids.map((u) => [u, 0]));
          const ties = new Map<string, number>(ids.map((u) => [u, 0]));
          for (let i = 0; i < N; i++) for (let j = i + 1; j < N; j++) {
            ...
            if (tie) { ties.set(a, ties.get(a)! + 1); ties.set(b, ties.get(b)! + 1); }
            else if (aWins) wins.set(a, wins.get(a)! + 1); else wins.set(b, wins.get(b)! + 1);
          }
          ...
          const pl = new Map<string, boolean>(ids.map((u) => [u, isPlacement(u)]));
          for (const u of ids) {
        ...
        if (!s1.length || !s2.length) { matchesSkipped++; continue; }
        const winnerSide: number | null = mt.winner_side ?? null;
        ...
        const plMap = new Map<string, boolean>([...s1, ...s2].map((u) => [u, isPlacement(u)]));
        const playerDeltas: Record<string, number> = {};
        for (const u of s1) playerDeltas[u] = shapeDelta(side1Delta, !isTie && side1Wins, plMap.get(u)!);
        for (const u of s2) playerDeltas[u] = shapeDelta(side2Delta, !isTie && !side1Wins, plMap.get(u)!);

        for (const u of s1) bump(u, playerDeltas[u], !isTie && side1Wins, isTie);
        for (const u of s2) bump(u, playerDeltas[u], !isTie && !side1Wins, isTie);
        ...
        if (isLinked && mt.paired_match_id) {
          ...
          for (const u of [...s1, ...s2]) flipped[u] = playerDeltas[u];
    ...
    for (const [u, e] of elo) {
      await client.query(
        `UPDATE users SET elo = $1, total_matches = $2, total_wins = $3, total_ties = $4 WHERE user_id = $5`,
        [e, TM(u), tw.get(u) ?? 0, tt.get(u) ?? 0, u]);
      usersUpdated++;
    }
    ...
    for (const [matchId, wb] of writeback) {
      await client.query(
        `UPDATE match_results
          ...
          WHERE match_id = $1`,
        [matchId, wb.delta_elo, JSON.stringify({ ...wb.patch, replayed: true })]);
    }
...
export async function restoreElo(): Promise<{ usersRestored: number; resultsRestored: number }> {
  const client = await pool.connect();
  try {
    await client.query('BEGIN');
    const { rows: hasU } = await client.query(`SELECT to_regclass('elo_backup_users') IS NOT NULL AS ok`);
    if (!hasU[0]?.ok) throw new Error('No elo backup exists — nothing to restore');
    const u = await client.query(
      `UPDATE users SET elo = b.elo, total_matches = b.total_matches,
              total_wins = b.total_wins, total_ties = b.total_ties
         FROM elo_backup_users b WHERE users.user_id = b.user_id`);
    const r = await client.query(
      `UPDATE match_results SET delta_elo = b.delta_elo, details = b.details
