...
// Intentionally identical in spirit to the POST /courses create payload so
// the mobile client can apply it to the builder form with no translation.
...
export interface ScannedTeebox {
  name: string;
  gender: 'male' | 'female';
...
export interface ScannedScorecard {
  courseName: string | null;
  city: string | null;
  state: string | null;
  numHoles: 9 | 18;
  teeboxes: ScannedTeebox[];
  warnings: string[];
}
...
export class ScorecardScanError extends Error {
  status: number;
...
const SUBMIT_TOOL = {
  ...
  description:
    'Return the golf course details transcribed from the scorecard photo. ' +
...
const SYSTEM_PROMPT =
  'You are a meticulous transcriber of golf scorecards. You are given a photo ' +
...
export async function scanScorecard(
  ...
  if (!data) throw new ScorecardScanError('The scanning service returned an unreadable response.', 502);
  ...
  const toolUse = Array.isArray(data.content)
    ? data.content.find((b: any) => b?.type === 'tool_use' && b?.name === 'submit_scorecard')
    : null;
 ...
 *  builder form. */
function normalize(input: any): ScannedScorecard {
  ...
  const numHoles: 9 | 18 = Number(input.numHoles) === 9 ? 9 : 18;

  const teeSetsIn: any[] = Array.isArray(input.teeSets) ? input.teeSets : [];
  if (teeSetsIn.length === 0) {
    throw new ScorecardScanError('No tee sets could be read from that photo. Try a clearer photo.', 422);
  }

  const teeboxes: ScannedTeebox[] = teeSetsIn.slice(0, 6).map((tb: any, ti: number) => {
    ...
    for (const h of holesIn) {
      ...
      if (hole_num == null) continue; // OUT/IN/TOTAL or garbage row
      holes.push({
        hole_num,
    ...
    if (holes.length === 0) warnings.push(`No hole data was read for the "${name}" tee.`);
