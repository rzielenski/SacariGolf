...
/** Minimal shape both pool and a transaction client satisfy. */
interface Executor {
...
function valueFor(row: any): number | null {
  if (row.total_score == null || row.par == null || !row.num_holes) return null;
 ...
 * the pool otherwise.
 */
export async function syncRoundNormalized(exec: Executor, roundId: string): Promise<void> {
  const { rows } = await exec.query(
    `SELECT ${ROUND_SCORE_COLS}
       FROM rounds r
  ...
  await exec.query(
    `UPDATE rounds SET normalized_to_par = $2 WHERE round_id = $1`,
    [roundId, valueFor(rows[0])],
  );
}
 ...
 */
export async function reconcileNormalizedScores(): Promise<number> {
  let filled = 0;
  for (let batch = 0; batch < 200; batch++) {   // hard cap: 200 * 1000 rounds
    const { rows } = await pool.query(
      `SELECT r.round_id, ${ROUND_SCORE_COLS}
         FROM rounds r
         JOIN teeboxes t ON t.teebox_id = r.teebox_id
         LEFT JOIN matches m ON m.match_id = r.match_id
    ...
    for (const row of rows) {
      ...
      if (v == null) continue;
      await pool.query(`UPDATE rounds SET normalized_to_par = $2 WHERE round_id = $1`, [row.round_id, v]);
      filled++;
    }
