...
export interface LeaderboardEntry {
  ...
  username: string;
  side: number;
  /** Hole-by-hole gross, in play order. null/0/absent = not yet posted. */
  hole_scores: (number | null)[] | null;
  /** Par for each hole, same order/length as the holes being played. */
  parByHole: number[];
  completed: boolean;
  /** Opaque passthrough for the client (avatar, crest, etc.) — not used here. */
...
export interface LeaderboardRow {
  ...
  points: number | null;
  completed: boolean;
  /** 1-based; ties share the lower number. */
  position: number;
  meta?: Record<string, unknown>;
}
...
function modStableford(score: number, par: number): number {
  const d = score - par;
  if (d <= -2) return 5;
  if (d === -1) return 2;
  if (d === 0) return 0;
...
export function computeLeaderboard(entries: LeaderboardEntry[], format: string): LeaderboardRow[] {
  ...
  const rows: LeaderboardRow[] = entries.map((e) => {
    ...
    for (let i = 0; i < scores.length; i++) {
      const s = scores[i];
      if (typeof s !== 'number' || s <= 0) continue;   // unplayed hole
      thru += 1;
      total += s;
      const par = e.parByHole[i];
      if (typeof par === 'number' && par > 0) {
        parThru += par;
        if (isStableford) points += modStableford(s, par);
      }
    }
    return {
  ...
  const started = (r: LeaderboardRow) => r.thru > 0;

  rows.sort((a, b) => {
    if (started(a) !== started(b)) return started(a) ? -1 : 1;   // unplayed → bottom
    if (isStableford) {
      if ((b.points ?? 0) !== (a.points ?? 0)) return (b.points ?? 0) - (a.points ?? 0);
    } else if (a.toPar !== b.toPar) {
      return a.toPar - b.toPar;
    }
    if (b.thru !== a.thru) return b.thru - a.thru;               // further along wins ties
  ...
  let pos = 0, lastKey: string | null = null, ordinal = 0;
  for (const r of rows) {
    ordinal += 1;
    if (!started(r)) { r.position = ordinal; continue; }
    const key = isStableford ? `p${r.points}` : `t${r.toPar}`;
    if (key !== lastKey) { pos = ordinal; lastKey = key; }
    r.position = pos;
  }
