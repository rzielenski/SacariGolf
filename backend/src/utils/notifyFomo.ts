...
import { divisionForElo } from '../routes/seasons';
...
export async function notifyMatchResolved(matchId: string): Promise<void> {
  try {
    const { rows: resRows } = await pool.query(
      `SELECT winner_side, details FROM match_results WHERE match_id = $1`, [matchId],
    );
    if (!resRows.length) return;
    const winnerSide: number | null = resRows[0].winner_side;
    const details = resRows[0].details ?? {};
    const playerDeltas: Record<string, number> = details.playerDeltas ?? {};

    const { rows: players } = await pool.query(
      `SELECT mp.user_id, mp.side, u.username, u.push_token, u.elo, u.is_bot
    ...
    for (const p of players) {
      if (p.is_bot || !p.push_token) continue;
      ...
      const opp = players.find((o: any) => Number(o.side) !== Number(p.side));
      ...
      const before = divisionForElo(beforeElo);
      const after = divisionForElo(afterElo);
      ...
      await sendPush([p.push_token], title, body, { type: 'match_resolved', matchId });
    }
  } catch (err) {
 ...
 */
export async function notifyCupEndingSoon(): Promise<void> {
  try {
    ...
    const weekStart: Date = wk[0].week_start;
    const weekEnd: Date = wk[0].week_end;
    const now: Date = wk[0].now;

    const hoursLeft = (weekEnd.getTime() - now.getTime()) / 3_600_000;
    if (hoursLeft > 6 || hoursLeft <= 0) return; // only in the final 6-hour window

    const weekKey = weekStart.toISOString();
    const { rows: cfg } = await pool.query(
      `SELECT value FROM app_config WHERE key = 'cup_reminder_week'`,
    );
    if (typeof cfg[0]?.value === 'string' && cfg[0].value === weekKey) return; // already sent
    ...
    const tokens = targets.map((t: any) => t.push_token).filter(Boolean);
    ...
    await pool.query(
      ...
      [JSON.stringify(weekKey)],
    );

    if (tokens.length) {
      await sendPush(
        tokens,
        '⛳ Sacari Cup ends tonight',
        'Last chance to top this week’s board. One good round could do it.',
        { type: 'cup_ending' },
      );
    }
  } catch (err) {
