...
export async function ensureCurrentCup(): Promise<void> {
  await pool.query(
...
export async function resolveFinishedCups(): Promise<number> {
  // Find finished-but-still-active cups (week ended at least 1s ago).
  const { rows: cups } = await pool.query(
  ...
  for (const cup of cups) {
    const ok = await resolveOne(cup.cup_id as string, cup.week_starts_at as Date);
    if (ok) resolved++;
  }
  return resolved;
}
...
async function resolveOne(cupId: string, weekStartsAt: Date): Promise<boolean> {
