...
export interface RatingSlopeEstimate {
  rating: number;        // 1-decimal, bounded [55, 80]
  slope: number;         // integer, bounded [85, 140]
  estimated: true;       // marker the caller can show in the UI
}
...
export interface CourseValidationResult {
  hardErrors: string[];   // refuse the submission
  warnings: string[];     // accept but flag in the UI
}
 ...
 */
export function validateTeebox(
  teeName: string,
  numHoles: number,
  holes: HoleInput[],
  declaredPar?: number | null,
  declaredYards?: number | null,
  ...
  for (const h of holes) {
    ...
    if (h.yardage != null) {
      if (!Number.isFinite(h.yardage) || h.yardage < 30 || h.yardage > 750) {
        ...
        if (h.par === 3 && h.yardage > 280) warnings.push(`${where}: par 3 at ${h.yardage} yds is on the long side.`);
        if (h.par === 4 && (h.yardage < 200 || h.yardage > 520)) warnings.push(`${where}: par 4 at ${h.yardage} yds is unusual.`);
        if (h.par === 5 && (h.yardage < 400 || h.yardage > 700)) warnings.push(`${where}: par 5 at ${h.yardage} yds is unusual.`);
      }
    }
