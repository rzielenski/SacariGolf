...
import {
  ...
  type CameraDevice, type CameraDeviceFormat,
...
function pickFormat(device: CameraDevice | undefined, fps: number): CameraDeviceFormat | undefined {
  ...
  // Filter to formats that can hit the requested fps.
  const usable = device.formats.filter((f) => f.maxFps >= fps && f.minFps <= fps);
  if (!usable.length) return undefined;
  // Prefer the largest 1080p-ish resolution; falls back to whatever the
  // device offers. 4K @ 240 isn't a real combo on any iPhone shipped,
  // so we don't try to chase it.
  usable.sort((a, b) => (b.videoWidth * b.videoHeight) - (a.videoWidth * a.videoHeight));
  // Cap at 1920x1080 — beyond that the storage hit on a 5s swing gets
  // wasteful for what we're using the footage for.
  return usable.find((f) => f.videoWidth <= 1920 && f.videoHeight <= 1920) ?? usable[0];
}
...
function deviceMaxFps(device: CameraDevice | undefined): number {
  ...
  return device.formats.reduce((max, f) => Math.max(max, f.maxFps), 0);
}
